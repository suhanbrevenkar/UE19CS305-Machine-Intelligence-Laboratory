...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        dt2=np.reshape(gk,(len(data),len(self.centroids)))

        for i in range(len(dt2)):
            clustassgn.append(np.argmin(dt2[i]))  
    ...
    def m_step(self, data, cluster_assgn):
        ...
        newcentr=np.zeros(shape=(r,c))
        j=0
        for i in cluster_assgn:
            newcentr[i]=np.add(newcentr[i],data[j])
            j+=1
        cluster_assgn=np.array(cluster_assgn)
        for k in range(len(newcentr)):
            count=(cluster_assgn==k).sum()
            newcentr[k]=newcentr[k]*(1/count)  

        self.centroids=newcentr
